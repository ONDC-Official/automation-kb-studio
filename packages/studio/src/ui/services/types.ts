/**
 * API contracts — the shapes the Studio's `/api` sends and accepts. Mirrors: packages/studio/src/server.ts
 * (and the engine's public shapes it re-serves). DELIBERATELY hand-copied and NOT imported from
 * `@evaluator/core`: this file compiles into a browser bundle, and importing the engine would drag the
 * OpenAI SDK (and Node built-ins) into it. When the server contract changes, edit here first and let
 * tsc find the call sites. Keep byte-compatible with the engine. Domain types get no `I` prefix.
 */
