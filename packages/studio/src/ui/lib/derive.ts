/**
 * Pure derivations shared by the pages, components, and tests. No React, no DOM, no fetch, no clock —
 * every function here is a total function of its arguments, so the grouping/filtering/health logic can
 * be unit-tested directly and reused between the author list, the tree spine, and coverage.
 *
...
/** Tailwind text-color classes per bucket. Components map a status/bucket → color through here so the
 *  fixed status palette (ok/warn/error tokens) stays consistent app-wide and never a one-off hex. */
export const BUCKET_TEXT: Record<StatusBucket, string> = {
  ok: "text-ok",
  caution: "text-warn",
  gap: "text-neutral-500",
  alarm: "text-error",
};

/** Tailwind dot/background classes per bucket (for status pips and chips). */
export const BUCKET_DOT: Record<StatusBucket, string> = {
  ok: "bg-ok",
  caution: "bg-warn",
  gap: "bg-neutral-400",
  alarm: "bg-error",
...
export const bucketOf = (status: TopicStatus): StatusBucket => STATUS_BUCKET[status];
...
function classify(from: TopicStatus, to: TopicStatus): { note: string; bucket: StatusBucket } {
  if (to === "canary-bit") return { note: "REGRESSION", bucket: "alarm" };
  if (from === "canary-bit" && to === "canary-ok") return { note: "fixed", bucket: "ok" };
  if (to === "grounded") return { note: "coverage gain", bucket: "ok" };
  if (from === "grounded") return { note: "regression", bucket: "caution" };
  return { note: "changed", bucket: "gap" };
}
...
export function statusTransitions(newer: CoverageReport, older: CoverageReport): Transition[] {
  ...
  for (const t of newer.topics) {
    ...
    const { note, bucket } = classify(prev.status, t.status);
    out.push({ key: k, id: t.id, path: t.path, title: t.title, from: prev.status, to: t.status, note, bucket });
  }
